...
const ScrollPlane = ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement | null> }) => {
  ...
  const x = useTransform(
    ...
    ["15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "15%", "85%", "50%"]
  );
  ...
  const rotate = useTransform(
    ...
    { clamp: false } // Allows rotation to accumulate
  );
  
  // Actually, let's use a simpler accumulation for rotation
  const totalRotate = useTransform(smoothProgress, (v) => v * 3600); // 10 full turns over the whole scroll
